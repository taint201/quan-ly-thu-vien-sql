...
-- ok-6.4 Liệt kê tổng số đầu sách của mỗi loại sách ( gồm mã loại sách, tên loại sách, tổng số 
--lượng sách mỗi loại).
select*from sach
select*from phanLoai
----
select a.maPhanLoai, b.tenPhanLoai, COUNT(b.maPhanLoai) as 'Số Đầu Sách' from sach a
join phanLoai b 
on a.maPhanLoai = b.maPhanLoai
group by a.maPhanLoai , b.tenPhanLoai
...
datediff(YEAR,PM.ngayTra,getdate()) as 'So Ngay vuot'
from phieuMuon PM
join sinhVien SV
on SV.maSV = PM.maSV
where trangThai like N'0'
--and datediff(YEAR,PM.ngayTra,getdate()) 
