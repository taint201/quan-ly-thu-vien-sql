...
from hoaDon hd
inner join HoaDonChiTiet hdct
on hdct.maHoaDon = hdct.maHoaDon
...
--toán.
select*from khachHang
select hoTenLot + ''+ tenKH  as 'Họ và Tên',email,dienThoai,hd.maHoaDon,trangThai,hdct.soLuong*donGia as 'Tổng Tiền' from khachHang kh
...
select * from hoaDon
select * from hoaDon--chua dung
where maKH not in (select  maKH from hoaDon where ngayMuaHang >= '2022-02-13')

select  maKH,trangThai  from hoaDon where ngayMuaHang >= '2022-02-10'
...
where b.ngayMuaHang like '2022-1%'
...
where hoaDon.ngayMuaHang like '2022-1%'
